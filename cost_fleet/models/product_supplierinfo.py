...
class SupplierInfo(models.Model):
    ...
    def get_lowest_cost_supplierinfo_line(self, product):
        product.ensure_one()
        supplierinfo_line_lowest_cost = list_supplierinfo_lines = self.get_supplierinfo_lines(product)
        if len(list_supplierinfo_lines) > 1:
            supplierinfo_line_lowest_cost = list_supplierinfo_lines.sorted(key=lambda r: r.price)[0]

        return supplierinfo_line_lowest_cost 
        #TODO - Calcular precio sin impuesto
